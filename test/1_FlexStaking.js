...
contract("Testing Flex Staking", accounts => {
    ...
    it('should create stake Pool', async () => {
        ...
        await flexStaking.CreateStakingPool(lockedToken.address, amount, rewardToken.address, amount, startTime, finishTime, APR, oneDay, twoDays, '1', '10', '0')
    })

    it('should create Pool with the same reward token', async () => {
        await lockedToken.approve(flexStaking.address, amount * 2, { from: projectOwner })
        const date = new Date()
        const startTime = Math.floor(date.getTime() / 1000) + 60
        date.setDate(date.getDate() + 3)   // add 3 days
        const finishTime = Math.floor(date.getTime() / 1000) + 60
        const oneDay = 60 * 60 * 24 // seconds
        const twoDays = 60 * 60 * 24 * 2
        await flexStaking.CreateStakingPool(lockedToken.address, amount, lockedToken.address, amount, startTime, finishTime, APR, oneDay, twoDays, '1', '10', '0')
    })
    ...
    it('should be greater than zero', async () => {
        ...
        await truffleAssert.reverts(
            flexStaking.CreateStakingPool(lockedToken.address, amount, rewardToken.address, amount, startTime, finishTime, '0', oneDay, twoDays, '1', '10', '0'),
            'The value should be greater than zero!')
        await truffleAssert.reverts(
            flexStaking.CreateStakingPool(lockedToken.address, '0', rewardToken.address, amount, startTime, finishTime, APR, oneDay, twoDays, '1', '10', '0'),
            'The value should be greater than zero!')
        await truffleAssert.reverts(
            flexStaking.CreateStakingPool(lockedToken.address, amount, rewardToken.address, amount, '0', finishTime, APR, oneDay, twoDays, '1', '10', '0'),
            'The value should be greater than zero!')
        await truffleAssert.reverts(
            flexStaking.CreateStakingPool(lockedToken.address, amount, rewardToken.address, amount, startTime, '0', APR, oneDay, twoDays, '1', '10', '0'),
            'The value should be greater than zero!')
        await truffleAssert.reverts(
            flexStaking.CreateStakingPool(lockedToken.address, amount, rewardToken.address, amount, startTime, finishTime, APR, oneDay, '0', '1', '10', '0'),
            'The value should be greater than zero!')
        await truffleAssert.reverts(
            flexStaking.CreateStakingPool(lockedToken.address, amount, rewardToken.address, amount, startTime, finishTime, APR, oneDay, twoDays, '1', '0', '0'),
            'The value should be greater than zero!')
    })
})